...
interface PaymentMethodsProps {
  /** Base amount before any optional fee — membership price or donation amount. */
  baseAmount: number
  ...
  /** Reference code the payer includes in the Zelle memo (generated per transaction). */
  zelleReferenceCode: string
  /** Accent color family for the active state — 'emerald' for signup, 'amber' for donations. */
  accent?: 'emerald' | 'amber'
}

/**
 * Shared payment step for the signup and donation flows.
...
export function PaymentMethods({
  ...
  const activeRing =
    accent === 'amber'
      ? 'border-amber-400 dark:border-amber-500 ring-1 ring-amber-200 dark:ring-amber-800/50'
      : 'border-emerald-400 dark:border-emerald-500 ring-1 ring-emerald-200 dark:ring-emerald-800/50'
  const activeDot = accent === 'amber' ? 'border-amber-500 bg-amber-500' : 'border-emerald-500 bg-emerald-500'
            ...
            >
              <span
                className={`shrink-0 w-4 h-4 rounded-full border-2 flex items-center justify-center transition-colors ${
                  active ? activeDot : 'border-stone-300 dark:border-stone-600'
                }`}
              >
              ...
              <div className="px-4 pb-4 pt-1 animate-in fade-in duration-150">
                ...
                {m.id === 'zelle' && (
                  <ZellePanel
                    config={zelleConfig}
                    amount={baseAmount}
                    referenceCode={zelleReferenceCode}
                  />
                )}
              </div>
            )}
...
function CardFields() {
  return (
    <div className="space-y-2.5">
      <input type="text" placeholder="Card number" maxLength={19} className="dropin-card-field" />
      <div className="grid grid-cols-2 gap-2">
        <input type="text" placeholder="MM / YY" maxLength={7} className="dropin-card-field" />
        <input type="text" placeholder="CVV" maxLength={4} className="dropin-card-field" />
      </div>
      <input type="text" placeholder="Name on card" className="dropin-card-field" />
    </div>
  )
}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start gap-2 text-xs text-stone-500 dark:text-stone-400">
        ...
        <span>
          Include the reference code in your transfer memo. Your payment stays{' '}
