...
function certBadge(status: string) {
  ...
  if (status === 'not_applicable') {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-stone-100 text-stone-500 dark:bg-stone-800 dark:text-stone-400">
...
export function LeadTeamView({
  ...
  const { stats, subordinates, role, leadName, assistantLeadName } = leadDashboard
  const hasCertTracking = stats.certified + stats.needsRecertification > 0
      ...
      <div className="mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 mt-2">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800">
            ...
            <span className="text-xs font-semibold text-emerald-800 dark:text-emerald-300">{leadName}</span>
            <span className="text-[10px] text-emerald-600 dark:text-emerald-500 font-medium">Lead</span>
          </div>
          {assistantLeadName && (
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-stone-100 dark:bg-stone-800 border border-stone-200 dark:border-stone-700">
              <Shield className="w-3.5 h-3.5 text-stone-500 dark:text-stone-400" />
              <span className="text-xs font-semibold text-stone-700 dark:text-stone-300">{assistantLeadName}</span>
              <span className="text-[10px] text-stone-500 dark:text-stone-400 font-medium">Asst. Lead</span>
            </div>
          )}
        </div>
      </div>
